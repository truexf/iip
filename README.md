...
1. 1字节数据帧类型标识，0表示请求尾帧(如果一个帧包含了完整的请求，则这个请求只有一帧，且为尾帧)，1表示片帧（片帧表示请求还未完成，还有后续帧），2表示响应尾帧，4表示部分响应尾帧
文本路径（只存在于请求帧，响应帧不含该字段。与unix路径格式相同，类似于url的path，用于指明请求的路径，限制不大于1024个字符）
2. \0 
3. 4字节流标识符（多路复用的流身份ID，无符号整数，请求方自增实现）
4. 4字节数据长度（限制一个帧的数据长度不能大于16MB）
5. 数据
